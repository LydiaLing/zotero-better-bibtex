...
class MarkupParser
  ...
  parseEndTag: (tag, tagName) ->
    ...
    if pos >= 0
      ...
      i = @stack.length - 1
      while i >= pos
        @handler.end(@stack[i]) if @handler.end
        i -= 1
  ...
  class AST
    ...
    @::re.leadingUnprotectedWord = ///^([#{@::re.Lu}][#{@::re.lcChar}]*)[#{@::re.whitespace}#{@::re.P}]///
    @::re.protectedWords = ///^(#{@::re.protectedWord})(([#{@::re.whitespace}])(#{@::re.protectedWord}))*///
    @::re.unprotectedWord = ///^[#{@::re.char}]+///
    ...
    plaintext: (text, pos) ->
      ...
      if l == 0 || @elems[0].children[l - 1].name != '#text'
